...
public class CalyshmakUpdate extends AppCompatActivity {
    EditText miles,km;
    Button ok;
    TextView car_name;
    ImageView car_image;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        setContentView(R.layout.activity_calyshmak_update);
       ...
       // Typeface trebu = Typeface.createFromAsset(getAssets(), "fonts/trebuc.ttf");
        ...
        settings.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                startActivity(new Intent(CalyshmakUpdate.this, Sazlamalar.class));
                //Animatoo.animateSwipeLeft(CalyshmakUpdate.this);
            }
        });
        ...
        Cursor cursor1=calyshmakDb.getSelect(gelenID);
        if(cursor1.getCount()==0){
            return;
        }
        while (cursor1.moveToNext()){
            String[] vl=cursor1.getString(2).split(",");
            String alarm=cursor1.getString(6);
            String kmm=cursor1.getString(8);
            String ml=cursor1.getString(9);
            ...
            if(alarm.equals("1")){
                check_alarm.setChecked(true);
                check_alarmTV="1";
            }

            for(int i=0;i<vl.length;i++){
                //motor_filtre,motor_yag,korobka,howa_filtre,salon_filtre,frion,check_alarm;
                if(vl[i].equals("1")){
                    motor_filtre.setChecked(true);
                    motor_filtreTV=getResources().getString(R.string.motor_filtre);
                }
                if(vl[i].equals("2")){
                    motor_yag.setChecked(true);
                    motor_yagTV=getResources().getString(R.string.motor_yag);
                }
                if(vl[i].equals("3")){
                    korobka.setChecked(true);
                    korobkaTV=getResources().getString(R.string.korobka_yag_we_filtr);
                }
                if(vl[i].equals("4")){
                    howa_filtre.setChecked(true);
                    howa_filtreTV=getResources().getString(R.string.howa_filtr);
                }
                if(vl[i].equals("5")){
                    salon_filtre.setChecked(true);
                    salon_filtreTV=getResources().getString(R.string.salon_filtr);
                }
                if(vl[i].equals("6")){
                    frion.setChecked(true);
                    frionTV=getResources().getString(R.string.frion);
                }
            }
        }
        ...
        ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (first.isEmpty()) {
                    Alert alert = new Alert(CalyshmakUpdate.this, "no", getResources().getString(R.string.attention), getResources().getString(R.string.error8));
                    alert.ShowDialog();
                    return;
                }

                if (second.isEmpty()) {
                    Alert alert = new Alert(CalyshmakUpdate.this, "no", getResources().getString(R.string.attention), getResources().getString(R.string.warning));
                    alert.ShowDialog();
                    return;
                }
                ...
                if (calyshylan.isEmpty()) {
                    Alert alert = new Alert(CalyshmakUpdate.this, "no", getResources().getString(R.string.error_title2), getResources().getString(R.string.error_title));
                    alert.ShowDialog();
                    return;
                }
                calyshylan = calyshylan.substring(0, calyshylan.length() - 1);

                CalyshmakDB db = new CalyshmakDB(CalyshmakUpdate.this);
                if (db.updateSt(id)) {
                    boolean isInsert = db.updateData(gelenID, calyshylan,  check_alarmTV, "0", second, Integer.parseInt(first));

                    if (isInsert) {
                        Toast.makeText(CalyshmakUpdate.this, getResources().getString(R.string.succes), Toast.LENGTH_SHORT).show();
                        onBackPressed();
...
//                        }
//                        CostsDB costsDB = new CostsDB(CalyshmakUpdate.this);
//                        boolean isLast = costsDB.insert(id, lastId, "2", gun, String.valueOf(ay), yyl, hepde);
//                        if (isLast) {
//                            CarDB db1 = new CarDB(CalyshmakUpdate.this);
//                            boolean isUpdate = db1.updateBenzin(id, miles.getText().toString());
//                            if (isUpdate) {
//                                Toast.makeText(CalyshmakUpdate.this, getResources().getString(R.string.succes), Toast.LENGTH_SHORT).show();
//                                mNotificationManager =
//                                        (NotificationManager) CalyshmakUpdate.this.getSystemService(Context.NOTIFICATION_SERVICE);
//
...
//                        }
                        ...
                        Alert alert = new Alert(CalyshmakUpdate.this, "no", getResources().getString(R.string.error_title2), getResources().getString(R.string.error_title3));
                        alert.ShowDialog();
                    }
                }
            }
    ...
    public void onBackPressed() {
        finish();
        if(type==null){
            Intent car = new Intent(CalyshmakUpdate.this, My_Costs.class);
            car.putExtra("id", id);
            startActivity(car);
        } else {
            if (type.equals("1")) {
                Intent car = new Intent(CalyshmakUpdate.this, Car_View.class);
                car.putExtra("id", id);
                startActivity(car);
            }
            if (type.equals("2")) {
    ...
    public void notif(){
        String CHANNEL_ID="1";
        CarDB carDB=new CarDB(CalyshmakUpdate.this);
        CalyshmakDB calyshmakDB=new CalyshmakDB(CalyshmakUpdate.this);
        ...
        while (cursor.moveToNext()) {
            ...
            if (cursor1.getCount() == 0) {
                ...
                while(cursor1.moveToNext()) {
                    ...
                    Intent snoozeIntent = new Intent(CalyshmakUpdate.this, Calyshmak.class);
                    snoozeIntent.setAction("ACTION_SNOOZE");
                    snoozeIntent.putExtra("id", cursor.getString(0));
                    PendingIntent snoozePendingIntent =
                            PendingIntent.getActivity(CalyshmakUpdate.this, 0, snoozeIntent, 0);


                    NotificationCompat.Builder mBuilder =
                            new NotificationCompat.Builder(CalyshmakUpdate.this, cursor.getString(1)+"_"+cursor.getString(2));
